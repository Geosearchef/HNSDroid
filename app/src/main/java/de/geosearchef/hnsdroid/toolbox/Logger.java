...
public class Logger {
	...
	public synchronized static void init(String programTitle) {
		if (fileWriter != null)
			return;
		...
		try {
			...
			initFileWriter(programTitle, date);
		} catch (IOException e) {
			System.err.println("Could not initialize logging!");
		}
	}

	private synchronized static void initFileWriter(String programTitle, Date date) throws IOException {
		if (loggingEnabled) {
			fileWriter = new FileWriter((date.getYear() + 1900) + "-" + (date.getMonth() + 1) + "-" + date.getDate() + "_" + (date.getHours() < 10 ? "0" + date.getHours() : date.getHours()) + "-" + (date.getMinutes() < 10 ? "0" + date.getMinutes() : date.getMinutes()) + "-" + (date.getSeconds() < 10 ? "0" + date.getSeconds() : date.getSeconds()) + ".log");
